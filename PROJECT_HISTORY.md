...
1. **Commodore 64/128**
   ...
   - VIC-II video chip (barevná paleta, registry)
   - PETSCII kódování
   - CRT cartridge formát (`cartconv -t pf`)

2. **Pagefox ROM architektura** *(zdokumentováno v docs/pagefox_memory_map.md)*
   - Bankswitching přes registr `$DE80` (6 módů: EPROM 79/ZS3/Cart RAM × bank 0/1)
   - Shadow register `$42` pro aktuální bank, `$43` pro uložený stav
   - `!pseudopc` bloky pro přesné mapování kódu na ROM offsety
   - RAM blok `$0800–$17FF` — kopie ROM `$B000–$BFFF`, vždy dostupná bez ohledu na bank
   - Bankswitch thunky na `$0FD0`, `$0FDC`... musí běžet z RAM
   - 4 build varianty: CS/DE × 9pin/24pin

3. **Pagefox formáty**
   - ZS font formát (proprietární, RLE komprimovaný)
   - PG grafický formát (1bpp, vlastní RLE)
   - PT text editor formát
   - Vizawrite kompatibilita
   - `zs-cs.bin` / `zs-de.bin` — binární font banky
...
5. **Development tools**
   - ACME cross-assembler (`!source`, `!pseudopc`, `!binary`, `!ifdef`, makra)
   - `cartconv -t pf` pro generování CRT
   - VICE emulátor pro debugging (`x64sc -cartcrt`, monitor `m $de80`)
   - Python pro reverse engineering
   - Binary patching nástroje

6. **Grafika a fonty**
   - 8×8 bitmap fonty (33 fontů ZS série)
   - Czech diacritics implementace
   - Font workflow: CharFox → unrle.py → joinzs_fix.py → zs-cs.bin
...
**Programové soubory:**
- 10× Python nástroje (`tools/*.py`)
- 7× Assembly souborů (`src/pg_main`, `pg_cs`, `pg_de`, `pg_kernal`, `pg_colors`, `pg_24`, `pg_sd2iec`)
- 2× Binární font banky (`src/zs-cs.bin`, `src/zs-de.bin`)
- CRT soubory generuje CI automaticky (nejsou v repozitáři)
- 6× CharFox soubory (`charfox/`)

**Fonty:**
- 33× PRG fontů (`fonts/prg/`) — ZS1–ZS211
- 33× PNG náhledů fontů (`fonts/png/`)

**Dokumentace:**
- 6× Markdown soubory (README, CLAUDE.md, AGENTS.md, PROJECT_HISTORY.md, UPDATE_HISTORY.md, docs/pagefox_memory_map.md)
- 3× PDF manuály (`docs/` — manuál, booklet, Inside_Pagefox_small.pdf)

**CI/CD a tooling:**
- 1× GitHub Actions workflow (`.github/workflows/ci.yml`)
- 4× Skills soubory (`skills/`)
- 3× GitHub konfigurace (`.github/`)
- 5× Batch/exe nástroje (`tools/pack.bat`, `unpack.bat`, `acme.exe`, ...)

**Celkem: ~120 souborů (bez build/)**
...
*Git branch: claude/document-project-history-Zntdi (rebasováno na main)*
*Poslední commit main: 6c2cde8 (24. března 2026)*
